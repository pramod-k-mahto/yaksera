    ...
    <section className="w-full bg-white px-6 py-16 md:px-12 lg:px-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 mb-14">
        <div>
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-[#0a1628] leading-tight">
        ...
        <div className="lg:max-w-sm xl:max-w-md mt-2">
          ...
          <div className="flex items-center gap-[3px] flex-wrap">
            {Array.from({ length: 36 }).map((_, i) => (
              <span
                key={i}
                className={`rounded-full ${i % 5 === 0 ? 'w-2 h-2 bg-red-500' : 'w-1.5 h-1.5 bg-gray-300'}`}
              />
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {industries.map(({ icon, title, description }) => (
          ...
          >
            ...
            {/* Title */}
            <h3 className="text-[#0a1628] text-lg font-extrabold group-hover:text-red-500 transition-colors duration-300">
              {title}
            </h3>
